<template>
  <div id="menu">
    <el-menu
      ref="menuHeader"
      :default-active="handleValue"
      class="el-menu-vertical-demo-header"
      :collapse="isCollapse"
      @open="handleOpen"
      @close="handleClose"
      @select="handleSelect"
      router
    >
      <el-menu-item :index="item.indexRouter" v-for="item in handleRouter" :key="item.indexRouter" :disabled="item.disabled">
        <el-icon>
...
const handleRouter = ref([{indexRouter: 'index', title:'172.30.24.16',disabled: true,icon:'Menu'},
              {indexRouter: 'home', title:'仪表板',disabled: false,classFooter: false,icon:'Menu'},
])
