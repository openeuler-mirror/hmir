...
lazy_static! {
    ...
    pub static ref OS_RELEASE: io::Result<OsInfo> = OsInfo::new();
}
...
impl OsInfo {
    ...
    pub fn new() -> io::Result<OsInfo> {
        let file = BufReader::new(open("/etc/os-release")?);
        Ok(OsInfo::from_iter(file.lines().flat_map(|line| line)))
    }

    /// Attempt to parse any `/etc/os-release`-like file.
    pub fn new_from<P: AsRef<Path>>(path: P) -> io::Result<OsInfo> {
        let file = BufReader::new(open(&path)?);
        Ok(OsInfo::from_iter(file.lines().flat_map(|line| line)))
    }
}

impl FromIterator<String> for OsInfo {
